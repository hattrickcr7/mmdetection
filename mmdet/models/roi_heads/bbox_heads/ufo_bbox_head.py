...
class UFOBBoxHead(BaseModule):
    ...
    def __init__(self,
                 ...
                 loss_img=dict(
                     ...
                     loss_weight=1.0),
                 init_cfg=None):
        super(UFOBBoxHead, self).__init__(init_cfg)
    ...
    @force_fp32(apply_to=('cls_score', 'bbox_pred', 'img_score', 'img_det'))
    def loss(self,
             cls_score,
             bbox_pred,
             img_score,
        ...
        img_score = F.softmax(img_score, dim=1)
        img_det_list = img_det.split([p.bboxes.shape[0] for p in sampling_results])
        final_det_score = []
        losses['loss_img'] = 0
        ...
        final_score_list = final_score.split([p.bboxes.shape[0] for p in sampling_results])
        ...
        losses['loss_img'] = losses['loss_img']/len(sampling_results)
        return losses
