...
class UFOHead(BBoxHead):
    ...
    def loss(self,
        ...
        ref_img_logits = ref_logits.split([p.bboxes.shape[0] for p in rois])
        ref_img_bbox_preds = ref_bbox_preds.split([p.bboxes.shape[0] for p in rois])
        ...
        if ref_logits is not None:
            ...
            if ref_logits.numel() > 0:
                ...
                if self.custom_activation:
                    acc_ = self.loss_cls.get_accuracy(ref_logits, labels)
                    losses.update(acc_)
                else:
                    losses['acc'] = accuracy(ref_logits, labels)
        if ref_bbox_preds is not None:
            ...
            if pos_inds.any():
                if self.reg_decoded_bbox:
                    ...
                    ref_bbox_preds = self.bbox_coder.decode(rois[:, 1:], ref_bbox_preds)
                if self.reg_class_agnostic:
                    pos_bbox_pred = ref_bbox_preds.view(
                        ref_bbox_preds.size(0), 4)[pos_inds.type(torch.bool)]
                else:
                    pos_bbox_pred = ref_bbox_preds.view(
                        ref_bbox_preds.size(0), -1,
                ...
                if torch.isnan(losses['loss_bbox']):
                    losses['loss_bbox'] = torch.tensor(0.0)
            else:
                losses['loss_bbox'] = ref_bbox_preds[pos_inds].sum()
